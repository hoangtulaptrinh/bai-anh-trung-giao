...
app.get("/api/get_new_item", (req, res) => {
  ...
  if (_.has(objDrag, [nameDrag, nameDrop]) === true) {
    arrElements = _.find(
      elements,
      (val) => val.name == objDrag[nameDrag][nameDrop]
    );
  }
  if (_.has(objDrag, [nameDrag, nameDrop]) === false) {
    arrElements = _.find(
      elements,
      (val) => val.name == objDrop[nameDrop][nameDrag]
    );
  }
  if (arrElements !== undefined) {
    ...
    if (idDrop < 4) {
      ...
      objData.Target = _.concat(objData.Target, {
        ...
        name: arrElements.name,
        url: arrElements.url,
        recipe: `${objData.Items[req.query.idDrag].name} + ${
          objData.Target[idDrop].name
        }`,
      });
    } else {
      objData.Items = _.concat(objData.Items, newItemsObj);
...
io.on("connection", (socket) => {
  ...
  socket.on("Client-send-data", (data) => {
    //Case 2: socket.emit là sever chỉ gửi cho chính người gửi lên thôi VD: A gửi => sever gửi lại A
    socket.emit("Sever-send-data", { target: "you", messenger: data });

    //Case 3: socket.broadcast.emit là sever cho tất cả mọi người trừ người gửi VD: A gửi => sever gửi lại cho tất cả mọi người trừ A
